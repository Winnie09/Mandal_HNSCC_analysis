...
saveRDS(Res$populationFit, 'lamian_populationFit.rds')
...
sink(paste0(pdir, '/lamian_XDEType_table.txt'))
print(table(XDEType))
table(clu)
sink()
...
write.csv(res, paste0(pdir, 'lamian_differential_genes.csv'))
...
saveRDS(goRes, paste0(pdir, '/lamian_goRes.rds'))
...
pdf(paste0(pdir, '/lamian_hm_GO_term5.pdf'), width = 6.8, height = 3.5)
print(plotGOEnrich(goRes))
dev.off()

pdf(paste0(pdir, '/lamian_hm_GO_term10.pdf'), width = 6.8, height = 7)
print(plotGOEnrich(goRes, n = 10))
dev.off()

pdf(paste0(pdir, '/lamian_hm_GO_term10_fdr0.25.pdf'), width = 5, height = 2)
print(plotGOEnrich(goRes, n = 10, fdr.cutoff = 0.25))
dev.off()

pdf(paste0(pdir, '/lamian_hm_GO_term10_sortbyFC_fdr0.25.pdf'), width = 9, height = 7)
...
# --------------------------------------
png(paste0(pdir, 'lamian_DiffFitHm5.png'),width = 2500,height = 2200,res = 300)
plotDiffFitHm5(Res, subsampleCell = TRUE)
dev.off()

Res$expr <- NULL
saveRDS(Res, paste0(rdir, paste0('lamian_numeric_res_with_clu.rds')))
...
pdf(paste0(pdir, 'lamian_cluster_mean.pdf'), width = 4.2, height = 3.5)
...
pdf(paste0(pdir, 'lamian_cluster_mean_abtype.pdf'), width = 3.8, height = 3.8)
plotClusterMean(testobj = Res, cluster = clutype, type = 'Variable', facet = TRUE, facet_scales = 'free', facet_nrow = 7)
dev.off()

pdf(paste0(pdir, 'lamian_cluster_group_difference.pdf'), width = 2.2, height = 7.5)
